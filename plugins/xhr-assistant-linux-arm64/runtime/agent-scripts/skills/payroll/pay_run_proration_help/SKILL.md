...
Pay run proration helps payroll teams understand adjustments for partial periods, employee changes, or other rules that affect the calculated amount. The proration audit and summary views are intended to make calculation adjustments easier to review.

Use proration details when a result looks different from the expected full-period amount. The exact calculation still depends on payroll setup, employee data, effective dates, pay components, and backend payroll rules.
