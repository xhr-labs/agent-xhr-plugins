...
class _StoreOptionalFloatAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        setattr(namespace, self.dest, float(_clean_cli_scalar(values)))
...
def build_task_args_from_cli(argument_specs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for spec in argument_specs:
        ...
        if action_type == CLI_INT:
            ...
            continue
        if action_type == CLI_FLOAT:
            parser.add_argument(option, dest=dest, action=_StoreOptionalFloatAction, required=required)
            continue
