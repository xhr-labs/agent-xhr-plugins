...
async def _resolve_employee_name(name, context: RequestContext, http_client: HttpClient):
    ...
    for item in items:
        ...
        if employee_name and employee_name.lower() == lowered_target:
...
async def run(task_args, context: RequestContext, http_client: HttpClient):
    ...
    shift_id = shift_resolution.get("shift_id")
    url = f"{api_base_url}/v1/atd/shifts/{shift_id}/employees"
    payload = {
    ...
    if status_code < 200 or status_code >= 300:
        return error_result(
            f"remove_employee_from_shift_failed:{status_code}:{json.dumps(response_payload, ensure_ascii=False)}"
        )
    ...
    return ok_result({
        ...
        "nextAction": "review_shift_assignment_status",
    })
