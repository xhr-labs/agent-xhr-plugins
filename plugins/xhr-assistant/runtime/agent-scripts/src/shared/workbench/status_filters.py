...
TASK_STATUS_KEY_PREFIX = "status.task."
TASK_STATUS_TYPE = "TaskStatus"


def is_task_status(status: Any) -> bool:
    """True only for statuses that apply to tasks.

    `/v1/pm/statuses` returns project statuses and task statuses in one list,
    and both scopes reuse display names (e.g. two "In Progress" entries).
    Sending a project status id to a task update fails with
    "Status change is not allowed by the task workflow", so task flows must
    never see the project-scope entries.
    """
    if not isinstance(status, dict):
        return False
    translate_key = (status.get("translate_key") or "").lower()
    if translate_key:
        return translate_key.startswith(TASK_STATUS_KEY_PREFIX)
    return status.get("status_type") == TASK_STATUS_TYPE
