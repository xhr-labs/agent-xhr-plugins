...
> "That's a payroll run question — let me hand that off to the right flow so you get the right answer. For now, once your workspace is configured here, head to [Payroll → Pay Runs](/apps/payroll/pay-runs) and Lumi will walk you through it from there."
...
3. **[Have someone reach out to me](/contact-support?topic=schedule_a_call)**
...
> Head to **[Time Off -> Configuration](/apps/leave/timeoff/configuration)** to set your leave types and balances.
...
Direct the user to **[People → Add Employee](/people)**. They can add manually or import via CSV. They must include salary, contract type, and payment details.
...
Direct them to **[Workspace → Work Locations](/workspace/work-locations)**. They need to add at least one office or remote site.
...
Direct them to **[Time Off → Configuration](/apps/leave/timeoff/configuration)**. They set leave types and balances here.
...
Direct them to **[Attendance Tracking → Configuration](/apps/atd/configuration)** to define working hours and overtime rules.
...
Direct them to **[Workspace → Access & Permissions](/workspace/access-and-permissions)**. This controls who sees salary data and ensures approvals route correctly.
...
Once the relevant steps above are complete, direct the user to **[Payroll → Pay Runs](/apps/payroll/pay-runs)**. Offer to walk them through it or suggest they book a call if they'd prefer a guided session.
...
| "Just want to run payroll now" | Acknowledge it, but clarify this flow covers workspace configuration first. Flag any incomplete required steps, then redirect to [Payroll → Pay Runs](/apps/payroll/pay-runs) once done. |
...
> "You're set. Head to [Payroll → Pay Runs](/apps/payroll/pay-runs) to run your first payroll — or I can take you there now."
...
> ---
> Prefer to talk it through? [Book a call](https://cal.x-hr.co/demo-bookings-calendar-91a3194205909a92) or [have someone reach out to you](/contact-support?topic=schedule_a_call).
