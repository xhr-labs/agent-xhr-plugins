...
description: Link or unlink Workbench tasks with dependency relations (blocks, is blocked by, relates to) or view existing linked tasks.
side_effect: write
---
...
- **Input Verification Rule**:
  - `task_id` and `target_task_id` must be valid UUIDs (if only task names are known, run `python skills/workbench/get_tasks/scripts/get_tasks.py --name "<task title or keyword>" --page-number 0` to resolve task UUIDs).
  - Use `action`: `get` (default), `link` (add link), or `unlink` (remove link).
  - When linking, `relation_type` accepts `blocks`, `is_blocked_by`, or `relates_to` (defaults to `relates_to`).
- **Mandatory User Confirmation & Turn Boundary (for link / unlink)**:
  - For write actions (`link` or `unlink`), **ALWAYS** present a preview:
    - **Source Task**: `<source task number & name>`
    - **Target Task**: `<target task number & name>`
...
- `relation_type`: Relationship type (`blocks`, `is_blocked_by`, `relates_to`).

### Execution
```text
python skills/workbench/manage_linked_tasks/scripts/manage_linked_tasks.py --task-id <UUID> [--action <get|link|unlink>] [--target-task-id <UUID>] [--relation-type <blocks|is_blocked_by|relates_to>]
```
