...
- **Status changes**: prefer `--status-name "<name>"` (for example `--status-name "In Progress"`); the script resolves the correct task-status UUID itself. When passing `--status-id`, use only ids whose `status_key` starts with `status.task.` (from `get_tasks.available_statuses` or `get_task_status_meta.task_statuses`) — the workspace also has PROJECT statuses that reuse the same display names (two different "In Progress" ids can exist) and the task API rejects them with "Status change is not allowed by the task workflow".
...
- `status_id`: UUID of the target task status (must be a `status.task.*` status).
- `status_name`: Display name of the target task status (e.g. `In Progress`); the script resolves the UUID and fails clearly when the name is unknown or ambiguous.
...
### Execution
```text
python skills/workbench/update_task/scripts/update_task.py --task-id <UUID> [--task-name "<name>"] [--status-name "<task status name>"] [--status-id <UUID>] [--priority <Low|Medium|High|Urgent>] [--assignee-id <UUID>] [--due-date <YYYY-MM-DD>]
```
