...
known `{{name}}` placeholders with `app_url` prefixed for relative routes, and leaves unknown placeholders unchanged.
...
- output values (JSON structure or text) have template placeholders bound with
  `app_url`.
...
APP_URL=<server-configured xHR frontend App URL>
```
